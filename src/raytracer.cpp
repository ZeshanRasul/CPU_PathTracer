...
void EvaluateGGX(float alpha, const glm::vec3& normal, const glm::vec3& halfVector, float& D, float& pdf, float t, glm::vec3 wi)
{
	float alphaSqr = alpha * alpha;
	float pi = (float)M_PI;
	float theta_h = glm::acos(glm::dot(halfVector, normal));
	float cos4 = glm::pow(glm::cos(theta_h), 4.0f);
	float tan2 = glm::pow(glm::tan(theta_h), 2.0f);
	float DNumerator = alphaSqr;
	float DDenominator = pi * cos4 * glm::pow(alphaSqr + tan2, 2.0f);
	D = DNumerator / DDenominator;
	float costheta = glm::dot(normal, wi);
	pdf = (1 - t) * (costheta / (float)M_PI) + t * (D * (glm::dot(normal, halfVector)) / (4.0f * glm::dot(wi, halfVector)));
}
...
glm::vec3 PathTracerFindColor(UniformGrid* grid, const Ray& ray, Scene* scene, Camera* camera, int depth, int samples, bool stratify, const Intersection& intersection, std::string useNEE, bool useRR, glm::vec3 throughput, std::string importanceSampling, std::string chosenBRDF)
{
		...
		for (QuadLight* light : lights)
		{
			...
			for (int i = 0; i < samples; i++)
			{
				...
				else if (intersection.brdf == "ggx")
				{
					...
					float NdotH = std::max(glm::dot(intersection.hitObjectNormal, halfVector), 0.0f);
					float NdotV = std::max(glm::dot(intersection.hitObjectNormal, wo), 0.0f);
					float NdotL = std::max(glm::dot(intersection.hitObjectNormal, wi), 0.0f);
					float VdotH = std::max(glm::dot(wo, halfVector), 0.0f);
					...
					pdfBRDF = (1 - t) * (NdotL / (float)M_PI) +
						t * D * (NdotH / (4.0f * VdotH + 1e-6f));
	...
	else if (importanceSampling == "brdf")
	{
		if (intersection.brdf == "phong")
		{
			...
			weight = (pDiffuse * pdfDiffuse + pSpec * pdfSpec) > 0.0f ? (pDiffuse * pdfDiffuse) / (pDiffuse * pdfDiffuse + pSpec * pdfSpec) : 0.0f;
		}
		else if (intersection.brdf == "ggx")
		{
			float ks_avg = (intersection.hitObjectSpecular.r + intersection.hitObjectSpecular.g + intersection.hitObjectSpecular.b) / 3.0f;
			...
			if (xi0 <= t)
			{
				...
				float fresnelR = intersection.hitObjectSpecular.r + (1.0f - intersection.hitObjectSpecular.r) * glm::pow(1.0f - glm::dot(wi, halfVector), 5.0f);
				float fresnelG = intersection.hitObjectSpecular.g + (1.0f - intersection.hitObjectSpecular.g) * glm::pow(1.0f - glm::dot(wi, halfVector), 5.0f);
				float fresnelB = intersection.hitObjectSpecular.b + (1.0f - intersection.hitObjectSpecular.b) * glm::pow(1.0f - glm::dot(wi, halfVector), 5.0f);
			...
			else
			{
				...
				float cosTheta = glm::sqrt(xi1);
				float sinTheta = glm::sqrt(1.0f - xi1);

				glm::vec3 localDir(
					glm::cos(phi) * glm::sin(theta_diffuse),
					glm::sin(phi) * glm::sin(theta_diffuse),
					glm::cos(theta_diffuse));
	...
	//float weightBRDF = 1.0f;

	//if (intersection.brdf == "ggx" && useNEE == "mis")
	//{
	//	weightBRDF = pdf_ggx * pdf_ggx / (pdf_ggx * pdf_ggx + pdfNEE * pdfNEE + 1e-6f);
	//}
	//else if (intersection.brdf == "phong" && useNEE == "mis")
	//{
	//	weightBRDF = pdf * pdf / (pdf * pdf + pdfNEE * pdfNEE + 1e-6f);
	//}
	//else
	//{
	//	weightBRDF = 1.0f;
			...
			else if (integrator == "pathtracer")
			{
				...
				for (int pixSample = 0; pixSample < spp; pixSample++)
				{
					...
					else
					{
						...
						accumCol += PathTracerFindColor(&grid, ray, &scene, &cam, depth, lightSamples, lightStratify, intersection, useNEE, useRR, throughput, importanceSampling, intersection.brdf);
